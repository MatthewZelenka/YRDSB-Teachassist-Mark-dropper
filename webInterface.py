...
def getCourseData(session:requests.Session, coursePageUrl:str):
    ...
    course = {
        ...
        "assignment":{list(row)[0].getText():{'Knowledge/Understanding':{"mark":eval(list(row)[1].getText().split("=")[0]) if list(row)[1].getText() != "" else None,"weight":list(row)[1].getText().split("=")[-1] if list(row)[1].getText() != "" else None}, 'Thinking':{"mark":eval(list(row)[2].getText().split("=")[0]) if list(row)[2].getText() != "" else None,"weight":list(row)[2].getText().split("=")[-1] if list(row)[2].getText() != "" else None}, 'Communication':{"mark":eval(list(row)[3].getText().split("=")[0]) if list(row)[3].getText() != "" else None,"weight":list(row)[3].getText().split("=")[-1] if list(row)[3].getText() != "" else None}, 'Application':{"mark":eval(list(row)[4].getText().split("=")[0]) if list(row)[4].getText() != "" else None,"weight":list(row)[4].getText().split("=")[-1] if list(row)[4].getText() != "" else None}} for row in tables[_courseTableIndex(tables=tables, header=['Assignment', 'Knowledge / Understanding', 'Thinking', 'Communication', 'Application'])].findAll('tr')[1:] if len(row) > 1
        }
    }
    # print(course)
    return course
