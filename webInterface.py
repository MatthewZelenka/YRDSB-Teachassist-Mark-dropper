    ...
    courses = [
            {
                ...
                "mark": float(list(row)[3].getText().split("current mark =")[-1].lstrip().rstrip().removesuffix('%')) if list(row)[3].getText().split("current mark =")[-1].lstrip().rstrip().removesuffix('%') and "Please see teacher for current status regarding achievement in the course" not in list(row)[3].getText().split("current mark =")[-1].lstrip().rstrip().removesuffix('%') else None,
...
def getCourseData(session:requests.Session, coursePageUrl:str):
    ...
    course = {
        "weighting":{
            list(row)[0].getText().replace(" ", ""): list(row)[-2].getText().removesuffix('%') for row in tables[_courseTableIndex(tables=tables, header=['Category', 'Weighting', 'Course Weighting', 'Student Achievement'])].findAll('tr')[1:]
        },
        "assignment":{
            list(row)[0].getText():{
                'Knowledge/Understanding':{
                    "mark":(eval(list(row)[1].getText().split("=")[0]) if list(row)[1].getText() != "" else None) if "no mark" not in list(row)[1].getText() else None,
                    "weight":((list(row)[1].getText().split("=")[-1] if list(row)[1].getText() != "" else None) if "no weight" not in list(row)[1].getText() else "0") if "no mark" not in list(row)[1].getText() else None
                }, 'Thinking':{
                    "mark":(eval(list(row)[2].getText().split("=")[0]) if list(row)[2].getText() != "" else None) if "no mark" not in list(row)[2].getText() else None,
                    "weight":((list(row)[2].getText().split("=")[-1] if list(row)[2].getText() != "" else None) if "no weight" not in list(row)[2].getText() else "0") if "no mark" not in list(row)[2].getText() else None
                }, 'Communication':{
                    "mark":(eval(list(row)[3].getText().split("=")[0]) if list(row)[3].getText() != "" else None) if "no mark" not in list(row)[3].getText() else None,
                    "weight":((list(row)[3].getText().split("=")[-1] if list(row)[3].getText() != "" else None) if "no weight" not in list(row)[3].getText() else "0") if "no mark" not in list(row)[3].getText() else None
                }, 'Application':{
                    "mark":(eval(list(row)[4].getText().split("=")[0]) if list(row)[4].getText() != "" else None) if "no mark" not in list(row)[4].getText() else None,
                    "weight":((list(row)[4].getText().split("=")[-1] if list(row)[4].getText() != "" else None) if "no weight" not in list(row)[4].getText() else "0") if "no mark" not in list(row)[4].getText() else None
                }
            } for row in tables[_courseTableIndex(tables=tables, header=['Assignment', 'Knowledge / Understanding', 'Thinking', 'Communication', 'Application'])].findAll('tr')[1:] if len(row) > 1
        }
    }
    # print(course)
    return course
...
if __name__ == '__main__':
    with requests.Session() as s:
        ...
        print(coursesData)
        print(getCourseData(session=s, coursePageUrl=coursesData[0]["url"]))
